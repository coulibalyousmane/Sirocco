...
// ROADMAP.md). Demontre ce que le format declaratif ne peut pas exprimer : une boucle de
// nouvelle tentative bornee sur "checkout", avec un arret anticipe dès qu'il ne s'agit plus
// d'une saturation temporaire (503).
//
    ...
    public async ValueTask ExecuteAsync(IVirtualUserContext context, CancellationToken cancellationToken)
    {
        ...
        if (token is null)
        {
            ...
            HttpResponseMessage loginResponse = await context.HttpClient.PostAsJsonAsync(
                "/api/auth/login", new { username = "demo", password = "demo" }, cancellationToken);
