...
Reste, comme un profil de charge à plusieurs paliers (`Sirocco:RampVus`), l'affaire d'un
...
`Sirocco.Host` n'a pas d'option pour désigner un fichier de configuration : il lit
...
Chaque entrée de `Sirocco:Scenarios` accepte le même vocabulaire que `SiroccoHostOptions` lui-même
...
Techniquement, `MultiScenarioRunner` (`Sirocco.Host.Execution`) construit à la main, pour chaque
...
`Sirocco.SampleTarget` : deux entrées indépendantes dans le rapport (100 puis 943 itérations,
