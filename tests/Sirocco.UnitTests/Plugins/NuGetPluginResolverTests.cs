...
public sealed class NuGetPluginResolverTests
{
    ...
    {
        (string feed, string cache) = CreateDirectories();
        BuildLocalPackage(feed, "Sirocco.Test.Plugin", "1.0.0", "old-version");
    ...
    public async Task A_source_that_lacks_the_package_falls_through_to_the_next_one()
    {
        ...
        string populatedFeed = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), $"sirocco-nuget-feed-{Guid.NewGuid():N}")).FullName;
    ...
    private static string CompileWorkflowAssembly(string workflowName)
    {
        ...
        string assemblyName = $"SiroccoNuGetPluginTest_{Guid.NewGuid():N}";
