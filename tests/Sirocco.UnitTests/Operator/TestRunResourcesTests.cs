using k8s;
using k8s.Models;
using Sirocco.Operator;
using Sirocco.Operator.Entities;

namespace Sirocco.UnitTests.Operator;
...
public sealed class TestRunResourcesTests
{
    ...
    {
        V1TestRun testRun = CreateTestRun();
        testRun.Spec.ClusterSharedSecretRef = new V1SecretKeySelector { Name = "sirocco-secret", Key = "shared-secret" };

        V1Job job = TestRunResources.BuildMasterJob(testRun);
        IList<V1EnvVar> env = job.Spec.Template.Spec.Containers[0].Env;

        V1EnvVar secretEnv = Assert.Single(env, e => e.Name == "Sirocco__ClusterSharedSecret");
        Assert.Null(secretEnv.Value);
        Assert.Equal("sirocco-secret", secretEnv.ValueFrom.SecretKeyRef.Name);
    ...
    public void Profile_stages_are_mapped_in_order()
    {
        ...
        Assert.Equal("0", Assert.Single(env, e => e.Name == "Sirocco__Profile__0__FromRps").Value);
        Assert.Equal("10", Assert.Single(env, e => e.Name == "Sirocco__Profile__0__ToRps").Value);
        Assert.Equal("10", Assert.Single(env, e => e.Name == "Sirocco__Profile__0__DurationSeconds").Value);
        Assert.Equal("10", Assert.Single(env, e => e.Name == "Sirocco__Profile__1__FromRps").Value);
        Assert.Equal("0", Assert.Single(env, e => e.Name == "Sirocco__Profile__1__ToRps").Value);
        Assert.Equal("5", Assert.Single(env, e => e.Name == "Sirocco__Profile__1__DurationSeconds").Value);
    }
