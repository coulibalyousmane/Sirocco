...
internal static class MetricFactory
{
    ...
    public static MetricResult Create(
        ...
        int virtualUserId = 0)
    {
        long responseTicks = TempestClock.FromSeconds(responseMilliseconds / MILLISECONDS_PER_SECOND);
        long serviceTicks = TempestClock.FromSeconds(serviceMilliseconds / MILLISECONDS_PER_SECOND);
