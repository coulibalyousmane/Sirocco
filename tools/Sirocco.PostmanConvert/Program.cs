...
using Sirocco.PostmanConvert;

// Convertit une collection Postman (export v2.1) en scenario scripte C# (.csx), directement
// jouable via WorkflowFileLoader (--scenario/Sirocco:ScenarioFile) sans aucun cablage
// supplementaire.

if (args.Length < 2)
{
    Console.Error.WriteLine("Usage : Sirocco.PostmanConvert <collection.json> <sortie.csx> [--name <nom>]");
    return 1;
}
