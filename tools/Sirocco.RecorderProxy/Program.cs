...
// HarConverter.Convert que Sirocco.HarConvert - aucune capture manuelle de HAR entre les deux.
...
catch (FormatException ex)
{
    ...
    Console.Error.WriteLine(
        "Usage : Sirocco.RecorderProxy --target-url <url> --out <scenario.csx> [--listen <url>] [--name <nom>]");
    return 1;
}
...
app.MapPost("/__sirocco-recorder/stop", (IHostApplicationLifetime lifetime) =>
{
...
Console.WriteLine("Ctrl+C pour arreter et generer le scenario (ou POST /__sirocco-recorder/stop).");
    ...
    HttpContext context, HttpClient httpClient, string targetUrl, List<HarEntry> capturedEntries, Lock capturedLock)
{
    ...
    catch (HttpRequestException ex)
    {
        ...
        await context.Response.WriteAsync($"Proxy Sirocco : echec de retransmission vers la cible : {ex.Message}", context.RequestAborted);
        return;
    }
