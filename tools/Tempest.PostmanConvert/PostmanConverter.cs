...
namespace Tempest.PostmanConvert;

/// <summary>
/// Traduit une collection Postman (export v2.1) en scenario scripte C# (<c>.csx</c>), meme
/// decision structurante que <c>Tempest.HarConvert</c>/<c>Tempest.OpenApiConvert</c> (voir
...
{
    // Meme raison que dans Tempest.HarConvert : geres autrement (Content-Type par le mode du
    ...
    private static string Render(string workflowName, List<ConvertedStep> steps)
    {
        ...
        code.AppendLine("// Genere par Tempest.PostmanConvert : les en-tetes d'authentification et les variables");
