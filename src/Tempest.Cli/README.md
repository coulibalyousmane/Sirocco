...
CLI de [Tempest](https://github.com/coulibalyousmane/Tempest), moteur de test de charge async
cloud-native en C#/.NET.

```bash
tempest run scenario.yaml --target-url http://localhost:5299 --rps 50 --duration 30s
```
...
`tempest run --help` documente toutes les options (`--threshold`, `--report-html`,
`--report-json`, `--max-vus`) et les limites de cette première version.

Le mode distribué (Master/Workers) reste l'affaire de `Tempest.Host` — voir le
[README du dépôt](https://github.com/coulibalyousmane/Tempest#readme).
