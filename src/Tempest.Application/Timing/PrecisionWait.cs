...
namespace Tempest.Application.Timing;
...
internal static class PrecisionWait
{
    ...
    /// </summary>
    /// <param name="targetTicks">Echeance, en ticks <see cref="TempestClock"/>.</param>
    ...
    public static void Until(long targetTicks, long spinThresholdTicks, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            long remaining = targetTicks - TempestClock.Now;
            if (remaining <= spinThresholdTicks)
            {
                break;
            }

            int sleepMilliseconds = (int)TempestClock.ToMilliseconds(remaining - spinThresholdTicks);
        ...
        while (TempestClock.Now < targetTicks)
        {
