...
/// <c>ClusterCertificatePinning</c> côté <c>Tempest.Host</c>.
/// <para>
...
public static class TestRunResources
{
    ...
    {
        ["tempest.dev/testrun"] = entity.Metadata.Name,
        ["tempest.dev/role"] = "master",
    };

    private static Dictionary<string, string> WorkerPodLabels(V1TestRun entity) => new()
    {
        ["tempest.dev/testrun"] = entity.Metadata.Name,
        ["tempest.dev/role"] = "worker",
    };
    ...
    private static List<V1EnvVar> BuildCommonEnv(V1TestRun entity)
    {
        List<V1EnvVar> env =
        [
            new V1EnvVar { Name = "Tempest__TargetBaseUrl", Value = entity.Spec.TargetBaseUrl },
        ];

        if (!string.IsNullOrEmpty(entity.Spec.Workflow))
        {
            env.Add(new V1EnvVar { Name = "Tempest__Workflow", Value = entity.Spec.Workflow });
        }
        ...
        if (entity.Spec.ClusterSharedSecretRef is { } secretRef)
        {
            ...
            {
                Name = "Tempest__ClusterSharedSecret",
    ...
    private static List<V1EnvVar> BuildMasterEnv(V1TestRun entity)
    {
        ...
        {
            env.Add(new V1EnvVar { Name = "Tempest__MaxVirtualUsers", Value = maxVirtualUsers.ToString() });
        }
        ...
        for (int i = 0; i < entity.Spec.Profile.Count; i++)
        {
            ...
            env.Add(new V1EnvVar { Name = $"Tempest__Profile__{i}__FromRps", Value = stage.FromRps.ToString() });
            env.Add(new V1EnvVar { Name = $"Tempest__Profile__{i}__ToRps", Value = stage.ToRps.ToString() });
            env.Add(new V1EnvVar { Name = $"Tempest__Profile__{i}__DurationSeconds", Value = stage.DurationSeconds.ToString() });
        }
    ...
    /// — aucun changement de code applicatif necessaire côté <c>Tempest.Host</c>.
    /// </summary>
    private static List<V1EnvVar> BuildWorkerEnv(V1TestRun entity)
    {
        ...
        env.Add(new V1EnvVar { Name = "Tempest__Role", Value = "worker" });
