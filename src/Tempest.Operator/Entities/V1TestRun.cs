...
namespace Tempest.Operator.Entities;
...
/// ressources Kubernetes qui le portent (voir <see cref="Tempest.Operator.TestRunResources"/>).
/// </summary>
[KubernetesEntity(Group = "tempest.dev", ApiVersion = "v1alpha1", Kind = "TestRun")]
public partial class V1TestRun : CustomKubernetesEntity<V1TestRun.TestRunSpec, V1TestRun.TestRunStatus>
{
    ...
    {
        /// <summary>Image (avec tag) de <c>Tempest.Host</c> à utiliser pour le maître et les workers.</summary>
        public string Image { get; set; } = string.Empty;

        /// <summary>URL de base de la cible du tir (<c>Tempest__TargetBaseUrl</c>).</summary>
        ...
        /// <summary>Nombre maximal d'utilisateurs virtuels (<c>Tempest__MaxVirtualUsers</c>).</summary>
        public int? MaxVirtualUsers { get; set; }

        /// <summary>Étapes du profil de charge (<c>Tempest__Profile__N__*</c>), au moins une.</summary>
        public List<ProfileStage> Profile { get; set; } = [];

        /// <summary>
        /// Workflow nommé à exécuter (voir <c>TempestHostOptions</c> : <c>websocket-echo</c>,
        ...
        /// (<c>Tempest__ClusterSharedSecret</c>). Jamais la valeur en clair dans la ressource.
        /// </summary>
